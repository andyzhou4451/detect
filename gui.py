...
class App(tk.Tk):
    def __init__(self):
        ...
        self.btn_plot = ttk.Button(self.ctrl_panel, text='绘制地图', command=self.run_plot)
        self.btn_plot.grid(row=6, column=0, pady=5, sticky='ew')
        ...
        self.progress.grid(row=8, column=0, pady=5, sticky='ew')

        self.canvas_orig = ttk.Label(self, text='')
        self.canvas_orig.configure(width=400, relief='solid')
        self.canvas_orig.grid(row=0, column=1, sticky='nsew')

        self.canvas_res = ttk.Label(self, text='')
        self.canvas_res.configure(width=400, relief='solid')
    ...
    def run_predict(self):
        ...
        try:
            ...
            self.progress.stop()
            messagebox.showerror('错误', f'分割失败: {exc}')
            self.set_status('分割失败', 'red')
            return
        self.progress.stop()
    ...
    def run_detect(self):
        ...
        if not os.path.exists(out_path):
            predict.process_image(self.img_path, out_path)
            if not os.path.exists(out_path):
                messagebox.showerror('错误', '检测所需的分割图像不存在')
                self.set_status('检测失败', 'red')
                return
        ...
        try:
            ...
            csv_path = os.path.join('csv_out', csv_name)
            coords_x, coords_y = detectX3.process_image(out_path, self.txt_path, out_csv=csv_path)
            img = Image.open(out_path)
        except Exception as exc:
            self.progress.stop()
    ...
    def run_plot(self):
        """Call the MATLAB plotting script."""
        self.set_status('绘图中...', 'orange')
        try:
            subprocess.run(['matlab', '-batch', 'ISW_figure'], check=True)
        except Exception as exc:
            messagebox.showerror('错误', f'绘图失败: {exc}')
            self.set_status('绘图失败', 'red')
            return
        self.set_status('绘图完成', 'green')
